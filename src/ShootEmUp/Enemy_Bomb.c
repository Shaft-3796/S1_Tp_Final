...
// Protos
void EnemyBomb_Delete(Enemy *self);
void EnemyBomb_Update(Enemy *self);
void EnemyBomb_Render(Enemy *self);
void EnemyBomb_Damage(Enemy *self, int damage);

Enemy *EnemyBomb_New(Scene *scene, Vec2 position, int life, float shoot_period)
{
    /* --- Base Ini --- */
    ...
    /* --- Functions bindings --- */
    self->Delete = &EnemyBomb_Delete;
    self->Update = &EnemyBomb_Update;
    self->Render = &EnemyBomb_Render;
    self->Damage = &EnemyBomb_Damage;
    /* --- --- --- --- --- --- --- */
    return self;
}
...
void EnemyBomb_Render(Enemy *self)
{
    ...
    pos_frame.x += BOMB_ENEMY_LIFE_BAR_OFFSET_X;
    pos_frame.y += BOMB_ENEMY_LIFE_BAR_OFFSET_Y;
    ...
    pos_content.x += BOMB_ENEMY_LIFE_BAR_OFFSET_X;
    pos_content.y += BOMB_ENEMY_LIFE_BAR_OFFSET_Y;
